 ...
 *
 * TODO(mattklein123): Currently, this performs a recursive updateMatchStatus() call on any child
 * match nodes. It's possible that we can short circuit this in certain cases but this needs more
 * thinking (e.g., if an OR matcher already has one match and it's not possible for a matcher to
 * flip from true to false). fixfix
 */
class Matcher {
public:
